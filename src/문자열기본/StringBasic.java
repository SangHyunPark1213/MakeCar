package 문자열기본;
// 자바의 문자열은 참조타입의 자료형, 객체를 만들어 사용하거나 리터럴 표기방식으로 사용할 수 있으면 리터럴 표기방식이 유리

public class StringBasic {
    public static void main(String[] args) {
        String name1 = "곰돌이사육사"; // 리터럴 표기방식
        String name2 = new String("곰돌이사육사"); // 쓰이지 않음

        // equals() : 두 개의 문자열을 비교하여 결과값을 리턴, 오버라이딩 되어있음
        System.out.println(name1.equals(name2)); // true
        System.out.println(name1 == name2); // 주소를 비교
        String text1 = "Spring";
        String text2 = "spring";
        ...
        System.out.println(text3.indexOf("Prog")); // 5, 없다면 -1
        ...
        // replace("old", "new") : 문자열 중에서 특정 문자열을 다른 문자열로 교체
        System.out.println(text3.replace("Java", "C++"));
        System.out.println(text3.replaceAll(" ", ""));
        // replaceAll() : 정규표현식 기반 치환

        // substring() : 문자열에서 특정 문자열을 추출할 때 사용
        System.out.println(text3.substring(5)); // 5 ~
        System.out.println(text3.substring(5, 10)); // 5 ~ 10 미만까지 추출

        // toUpperCase(), toLoswerCase() : 문자열을 전부 대문자 / 소문자로 변경
        ...
        String time = "10:32:34";
        String[] timeArr = time.split(":"); // :(콜론) 기준으로 여러개의 부분 문자열로 나누어 배열에 저장
        System.out.printf("%s시 %s분 %s초", timeArr[0], timeArr[1], timeArr[2]);

        // 문자열 포매팅 : 문자열 내에서 특정 값을 삽입하는 방법 : printf()와 유사
        int num = 4;
