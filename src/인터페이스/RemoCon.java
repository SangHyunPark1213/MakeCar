package 인터페이스;
// 인터페이스 : 클래스가 구현해야 할 메서드들의 설계명세서
// - "이런 기능을 제공해야 한다."는 규칙 또는 약속을 정의 (강제성)
// - 이를 통해서 다양한 클래스가 동일한 기능을 일관된 형식으로 구현
// - 기본적으로 모든 메서드가 추상 메서드임 (예외사항 존재)
// - 모든 필드는 상수로 변경 (public static final이 자동 추가)
// - 다중 구현 가능 (여러 개의 implements 가능)

public interface RemoCon {
    int MAX_VOLUME = 100; // 자동으로 public static final, 상속 안됨
    int MIN_VOLUME = 0;
    void turnON(); // 자동으로 public abstract
    void turnOFF();
    void setVolume(int volume);

    static void changeBattery() { // static을 붙이면 상속이 일어나지 않기 때문에 구현부가 존재
        System.out.println("건전지를 교환합니다.");
    }
}
